...
- `src/roster.ts` owns the fixed four-seat lobby. `LocalStorageRosterStore` persists `{version: 1, seats}` under `semanticspy.roster.v1`; handles remain internal and are rotated on same-name agent recovery. `MemoryRosterStore` is used by focused tests.
- `src/game-controller.ts` is the single browser state boundary. It exposes board reads, human/agent actions, registration/recovery, role alignment, New Game, and Next Round.
